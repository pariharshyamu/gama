const t=(o=0,a=22,s=20,r=0)=>`import { AmbientLight, Color, DirectionalLight, GridHelper } from 'three';
...
game.camera.position.set(${o}, ${a}, ${s});
game.camera.lookAt(0, ${r}, 0);`,e=t(),n=[{id:"seek",title:"Seek vs Arrive",group:"Steering",code:`// Two agents, one target. Blue SEEKS at full speed (and orbits —
...
game.start();`},{id:"wander",title:"Wander + Containment",group:"Steering",code:`// Aimless-but-organic meandering: each agent steers toward a point
...
game.start();`},{id:"pursuit",title:"Pursue & Evade",group:"Steering",code:`// The red hunter PURSUES the green runner's predicted position;
...
game.start();`},{id:"flock",title:"Flocking (200 boids)",group:"Steering",code:`// Classic boids: separation + alignment + cohesion, with a SpatialGrid
...
game.start();`},{id:"avoid",title:"Obstacle avoidance",group:"Steering",code:`// Agents ping-pong across a field of pillars. ObstacleAvoidance probes
...
game.start();`},{id:"path",title:"Path following",group:"Steering",code:`// A looping patrol circuit. FollowPath seeks each waypoint in turn;
...
game.start();`},{id:"navmesh",title:"Navmesh click-to-move",group:"Navigation",code:`// A donut-shaped walkable area (center blocked). Click anywhere:
...
game.start();`},{id:"navgen",title:"Navmesh generation",group:"Navigation",code:`// No hand-authored floor: the walkable surface (green) is BAKED from
...
game.start();`},{id:"behavior-tree",title:"Behavior tree guards",group:"AI",code:`// Guards run a reactive behavior tree: chase the target when it's
...
game.start();`},{id:"third-person",title:"Third-person template",group:"Templates",code:`// A playable third-person character in ONE call: shoulder camera with
...
game.start();`},{id:"npcs",title:"NPC archetypes",group:"Templates",code:`// Guards, a companion, and a flock — each one call. Drive the blue
...
game.start();`},{id:"character",title:"Character + camera + pickups",group:"Gameplay",code:`// Click the preview to focus it, then drive with WASD / arrows.
...
game.start();`},{id:"orbit",title:"Orbit camera rig",group:"Gameplay",code:`// Drag to orbit, wheel to zoom — smoothed, with pitch/distance limits.
...
game.start();`},{id:"tweens",title:"Tweens & easing",group:"Gameplay",code:`// One cube per easing function, bouncing forever. Tweens animate any
...
game.start();`},{id:"audio",title:"Procedural audio",group:"Audio",code:`// EVERY SOUND HERE IS SYNTHESIZED FROM A SEED — no audio files.
...
game.start();`},{id:"juice",title:"Game feel + HUD",group:"Feel",code:`// JUICE: the few milliseconds that make a hit feel like a hit, and
...
game.start();`},{id:"loot",title:"Collector + CheckpointRun",group:"Gameplay",code:`// THE PICKUP LOOP, END TO END. Collector sweeps the runner against
...
game.start();`},{id:"arena",title:"Health + Projectiles",group:"Gameplay",code:`// STAKES, wholesome register: bonk and knockout, not gore. The turret
...
game.start();`},{id:"waves",title:"WaveDirector + Harass",group:"Gameplay",code:`// THE OPPOSITION, PACED. The director decides when, how many, and how
...
game.start();`},{id:"trial",title:"Time trial: flow, ghost & the save",group:"Gameplay",code:`// THE RETENTION LOOP: beat yesterday's you. GameFlow runs the shell
...
game.start();`},{id:"coinrun",title:"Coin run: the payoff platformer",group:"Gameplay",code:`// THE PAYOFF: every pillar in one platformer. PlatformerController
...
game.start();`},{id:"stealth",title:"Stealth: the illumination field",group:"Gameplay",code:`// LIGHT AS GAMEPLAY. The stealth genre is one number: how lit am I?
...
game.start();`},{id:"aviator",title:"Aviator: the flight model",group:"Gameplay",code:`// ARCADE-HONEST FLIGHT. Throttle buys speed, speed buys lift,
...
game.start();`},{id:"rescue",title:"Night rescue: the searchlight",group:"Gameplay",code:`// THE SEARCHLIGHT HANDSHAKE. A HoverController flies the ship —
...
game.start();`},{id:"dogfight",title:"Dogfight: missiles, locks & flares",group:"Gameplay",code:`// THE TURN-RATE LIMIT IS THE WHOLE GAME. Missiles chase with lead
...
game.start();`},{id:"level",title:"Levels: prefabs & the round trip",group:"Core",code:`// A LEVEL, AND A LITTLE EDITOR. The scene on the left is built from the
...
};`},{id:"assets",title:"Assets: a manifest, groups & instancing",group:"Core",code:`// AN ASSET PIPELINE. Every model, texture and sound here is a real file
...
});`},{id:"net",title:"Multiplayer: prediction & interpolation",group:"Core",code:`// AUTHORITATIVE MULTIPLAYER, ALL IN ONE TAB. There is a real server here —
...
window.netDebug = () => ({
  ...
  draws: game.renderer.info.render.calls,
});`},{id:"dialogue",title:"Dialogue: a conversation as data",group:"Core",code:`// A CONVERSATION AS DATA. The script below is JSON — no functions in it —
...
window.dialogueDebug = () => ({
  ...
  draws: game.renderer.info.render.calls,
});`},{id:"railway",title:"Railway: the driver",group:"Gameplay",code:`// A train is the one vehicle that does not steer. Its entire
...
game.start();`},{id:"forage",title:"Utility AI: Charnov instead of a threshold",group:"AI",code:`// WHEN TO STOP IS THE HALF NOBODY HAS A PRINCIPLE FOR.
//
...
game.start();
`},{id:"flow",title:"Flow fields: the eight-way grid is 8.24% wrong",group:"AI",code:`// ONE FLOOD, ANY NUMBER OF AGENTS — AND THE USUAL ONE IS BIASED.
//
...
game.start();
`},{id:"voice",title:"Voice: the vowel chart IS the formant table",group:"AI",code:`// A VOICE WITHOUT AN AUDIO FILE.
//
...
game.start();
`},{id:"prosody",title:"Prosody: English rhythm is a published number",group:"AI",code:`// THE SAME WORDS, THREE DELIVERIES.
//
...
game.start();
`},{id:"consonants",title:"Consonants: a stop is a transition",group:"AI",code:`// A CONSONANT IS MOSTLY NOT A SOUND.
//
...
game.start();
`},{id:"diction",title:"Diction: text in, speech out",group:"AI",code:`// AN NPC THAT TALKS, WITH NO AUDIO FILES.
//
...
game.start();
`},{id:"tts",title:"The platform speaks, the mouth is ours",group:"AI",code:`// TWO WAYS TO MAKE AN NPC TALK, AND THIS PICKS BOTH.
//
...
game.start();
`}];function i(o){return n.find(a=>a.id===o)??n[0]}export{n as E,i as f};
