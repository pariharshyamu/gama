...
// bare 'gama'/'three' imports to the vendored ESM bundles, and runs it as a
// blob module. Each run gets a fresh iframe (the parent reloads us), so
...
window.addEventListener('message', async (event) => {
  ...
  const gamaUrl = vendor('gama.js');
  const templatesUrl = vendor('templates.js');
